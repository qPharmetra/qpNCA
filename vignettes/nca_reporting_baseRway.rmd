---
title: "nca_reporting_with_baseRbasedoldfunctions.rmd"
...
Theoph1 <- Theoph %>% 
  ...
  mutate(Subject=as.numeric(as.character(Subject)), #converting from factor to numeric
         ...
         conc= ifelse(conc<=0.25, 0, conc))      #just adding few BLQs to demonstrate functionality

#this is the nca analysis results file
dat <- read.csv("nca_analysis_stepbystep_results.csv", stringsAsFactors = F, na.strings = "NA")

#Units 
parUnits <- read.csv("Units_Template.csv", stringsAsFactors = F, na.strings = "")
```
...
#choose parameters wanted in the table
...
kable(nca_sum_formatted, row.names=F)
```
