...
qpNCA <- function(
  ...
  par_all = left_join(ctmax, par_all, by = by) %>%
    mutate(
      ...
      ctau = ifelse(tolower(reg) == "sd", NA, tauval),
      pctptftau = ifelse(tolower(reg) == "sd", NA, 100 * (cmax - cmin) / ctau),
      swing = ifelse(tolower(reg) == "sd", NA, 100 * (cmax - cmin) / cmin),
      swingtau = ifelse(tolower(reg) == "sd", NA, 100 * (cmax - ctau) / ctau)
    )
...
#' @param infdur variable containing infusion duration
#' @param factor conversion factor for CL and V calculation (e.g. dose in mg, conc in ng/mL, factor=1000); x$factor overrides if provided
#' @param reg regimen, "SD" or "MD"; x$reg overrides if provided
#' @param ss is steady state reached (y/n); x$ss overrides if provided
#' @param route route of drug administration ("EV","IVB", "IVI"); x$route overrides if provided
...
check.input <- function(
  ...
  chkfile <- data.frame(Errors_Warnings = "delete", stringsAsFactors = F)

  stop_by = 0 # used to tackle premature errors due missing by-variable in input frame
  ...
  if (!is.na(by[1]) & length(setdiff(by, names(x))) > 0) {
    chkfile = rbind(
      chkfile,
      paste(
        "Error: By variable(s) not in input data:",
        paste(setdiff(by, names(x)), collapse = ' ')
      )
    )
    stop_by = 1
  }
  if (!is.na(by[1]) & length(setdiff(by, names(get(covariates)))) > 0)
    chkfile = rbind(
      chkfile,
      paste(
        "Error: By variable(s) not in covariate dataframe:",
        paste(setdiff(by, names(get(covariates))), collapse = ' ')
      )
    )
  ...
  if (length(checknames[is.na(checkvalues)]) > 1)
    chkfile = rbind(
      chkfile,
      paste("Error: Arguments", misstext, "are mandatory")
    )
  ...
  if (!(all(checkvalues[!is.na(checkvalues)] %in% names(x))))
    chkfile = rbind(
      chkfile,
      paste(
        "Error: These column(s) are not in input data:",
        paste(
          setdiff(checkvalues[!is.na(checkvalues)], names(x)),
          collapse = ' '
        )
      )
    )
  ...
  if (!is.na(exclvar) & !(exclvar %in% names(x)))
    chkfile = rbind(
      chkfile,
      paste("Error: Exclusion variable (", exclvar, ") not in input data")
    )
  ...
  if (any(!is.na(includeCmax) & !(includeCmax %in% c("Y", "y", "N", "n"))))
    chkfile = rbind(
      chkfile,
      "Error: IncludeCmax argument can only be Y (default) or N"
    )
  ...
  if (!(missing(covariates))) {
    if (is.character(covariates)) {
      if (!exists(covariates)) {
        chkfile = rbind(
          chkfile,
          paste("Error: Covariate dataframe", covariates, "does not exist")
        )
      } else {
    ...
    if (!(dose %in% names(covariates))) {
      chkfile = rbind(
        chkfile,
        paste("Error: Dose variable (", dose, ") not in covariate dataframe")
      )
    }
    if (all(route %in% c("ivi", "IVI")) & !(infdur %in% names(covariates))) {
      chkfile = rbind(
        chkfile,
        paste(
          "Error: Infusion duration variable (",
          infdur,
          ") not in covariate dataframe"
        )
      )
    }
    if (length(setdiff(names(covariates), names(x))) > 0) {
      chkfile = rbind(
        chkfile,
        paste(
          "Error: Covariate(s) not in input data:",
          paste(setdiff(names(covariates), names(x)), collapse = ' ')
        )
      )
    }
  } else {
  ...
  if (any(is.na(factor)))
    chkfile = rbind(
      chkfile,
      "Warning: Factor argument is empty, assuming a value of 1"
    )
  ...
  if (any(ss %in% c("y", "Y") & is.na(tau)))
    chkfile = rbind(
      chkfile,
      "Warning: Tau not defined while at steady state, no clearances or volumes will be calculated"
    )
  ...
  if (any(tstart == 0 & !is.na(tstart)))
    chkfile = rbind(
      chkfile,
      "Error: Tstart cannot be 0, use Teval to calculate auc(0-Teval)"
    )
  ...
  if (stop_by != 1) {
    if (any(duplicated(x[c(by, nomtimevar)])))
      chkfile = rbind(
        chkfile,
        "Error: Records with duplicate nominal times present"
      )
  }
  ...
  if (stop_by != 1) {
    ...
    if (nrow(tst) > 0) {
      chkfile = rbind(
        chkfile,
        "Error: Some curves are not sorted by nominal time variable"
      )
    }
  }
  ...
  if (nrow(chkfile) > 0) {
    ...
    if (nrow(tst) > 0) {
      message("\n")
      message("Curves not sorted by nominal time variable:")
      print(kable(tst))
    }
    message("\n")
    if (any(grepl("Error", chkfile$Errors_Warnings)))
      stop("Execution of qPNCA aborted due to errors", call. = F)
  } else message("all OK!\n")
}
