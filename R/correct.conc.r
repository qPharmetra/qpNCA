...
#  th=NA,
  ...
  if(!'conc.tau' %in% names(x))stop('conc.tau not found; probably you need to run correct.time()')
...
if (!is.na(teval)) {
  ...
  if (!is.na(tau)) {
    x %<>% mutate_cond(
      ...
      crule.nr="MDC-1",
      crule.txt = paste(
        "Missing concentration at t=",tau,"
        corrected to ",
        round(conc.tau,3),
  ...
  if (!is.na(tstart)&!is.na(tend)) {
    x %<>%  mutate_cond(
     ...
     crule.nr="MDC-1",
     crule.txt=paste(
      "Missing concentration at t=",tend," corrected to ",
      round(conc.part,3),
      " by imputation with existing concentration at t=0",sep=""
      ),
