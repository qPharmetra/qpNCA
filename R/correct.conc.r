...
if (!is.na(teval)) {
  ...
  if (!is.na(tstart) & !is.na(tend)) {
    ...
    x %<>% mutate_cond(
      ...
      crule.nr=ifelse(tolower(reg)=="sd","SDC-2","MDC-2"),
      crule.txt=paste(
        "Missing concentration at t=",tstart,
    ...
    if (!is.na(tstart) & !is.na(tend)) {
      x %<>%  mutate_cond(
        ...
        crule.nr="MDC-1",
        crule.txt=paste(
          "Missing concentration at t=",tend," corrected to ", round(conc.part,3),
          " by imputation with existing concentration at t=0",sep=""
        ),
    ...
    x %<>% mutate_cond(
      ...
      crule.nr=ifelse(tolower(reg)=="sd","SDC-2","MDC-2"),
      crule.txt=paste(
        "Missing concentration at t=",tend,
        " corrected to ",round(conc.part,3),
        " by interpolation",sep=""
      ),
      applies.to.conc=paste(applies.to.conc,"TEND ")
  ...
  if (!is.na(teval)) {
    x %<>%  mutate_cond(
      ...
      crule.nr="MDC-1",
      crule.txt=paste(
        "Missing concentration at t=",teval,
        " corrected to ",round(conc.part,3),
        " by imputation with existing concentration at t=0",sep=""
      ),
  ...
  x %<>% mutate_cond(
    ...
    crule.nr="MDC-1",
    crule.txt=paste(
      "Missing concentration at PREDOSE corrected to ",round(conc.tau,3),
  ...
  x %<>% mutate_cond(
    ...
    crule.nr="SDC-1",
    crule.txt=paste(
      "Missing or measurable concentration at (SD) PREDOSE set to 0",sep=""
    ),
    applies.to.conc=paste("PREDOSE")
  )
  x %<>% mutate_cond(
  ...
  x %<>%  mutate_cond(
    ...
    crule.nr="MDC-1",
    crule.txt=paste(
      "Missing concentration at PREDOSE corrected to ",round(conc.teval,3),
      " by imputation with existing concentration at t=TAU",sep=""
    ),
    applies.to.conc=paste("PREDOSE")
  )
