...
correct.time <- function(
  ...
  if (is.null(th) & !'lambda_z' %in% names(x))
...
.correct.time <- function(
  ...
  x <- x |> mutate(
      ...
      tstart.flag = 0,
      tend.flag = 0,
      teval.flag = 0, # flags for what timepoint the correction was needed
    ...
    for (i in c(0, tau, tstart, tend, teval)) {
      if (!is.na(i)) {
        x <- x |> mutate(diff = abs(ptime - i)) |> # take closest neighbouring records as base for extra record
          ...
          mutate(
            misstime = ifelse(
              !is.element(i, ptime) & ptime == first(ptime[diff == min(diff)]),
              i,
              misstime
            )
          )
  ...
  x <- x |> # TAU
    ...
    mutate_cond(
      ...
      trule.txt = paste(
        ...
        " ) corrected to ",
        round(conc.tau, 3),
        " by interpolation (sample taken too early)",
        sep = ""
      ),
    ...
    mutate_cond(
      ...
      trule.txt = paste(
        ...
        " ) corrected to ",
        round(conc.tau, 3),
        " by interpolation (sample taken too late)",
        sep = ""
      ),
      applies.to.time = paste(applies.to.time, "TAU ")
    ) |>
    mutate_cond(
      ...
      trule.txt = paste(
        ...
        " ) corrected to ",
        round(conc.tau, 3),
        " by extrapolation (sample taken too early)",
        sep = ""
      ),
      applies.to.time = paste(applies.to.time, "TAU ")
    ) |>
    ...
    mutate_cond(
      ...
      time.part = 0,
      time.teval = 0,
      time.lastall = 0, # set predose time to 0
      conc.tau = depvar,
      conc.part = depvar,
      conc.teval = depvar,
      conc.lastall = depvar,
    ...
    mutate_cond(
      ...
      conc.tau = newdepvar,
      conc.part = newdepvar,
      conc.teval = newdepvar,
      conc.lastall = newdepvar,
      trule.nr = "MDT-3",
      trule.txt = paste(
        "Concentration at predose ( ",
        depvar,
        " ), taken before dosing ( t=",
        timevar,
        " ) corrected to ",
        round(newdepvar, 3),
        " by extrapolation (sample taken too early)",
    ...
    mutate_cond(
      ...
      conc.part = NA,
      conc.teval = NA,
      conc.lastall = NA, # set conc to NA
      trule.nr = "MDT-1",
      trule.txt = paste(
        "Concentration at predose ( ",
        depvar,
        " ), taken after dosing ( t=",
        timevar,
        " ) set to NA (sample taken too late)",
        sep = ""
      ),
      applies.to.time = "PREDOSE"
    ) #|>
