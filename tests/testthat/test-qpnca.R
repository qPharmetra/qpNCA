...
reference_results <- function(x, id, from, to, route){
  ...
  y %<>% select(id,parameter,value_excel)
  suppressWarnings(y$value_excel %<>% as.numeric %>% round(4))
  y
}

merged_results <- function(x, y){
  x %<>% left_join(y)
  x %<>% mutate(identical = as.integer(value_test == value_excel))
  x %<>% mutate(identical = ifelse(is.na(value_excel) & is.na(value_test),1,identical))
...
test_that('PO SD results are stable',{
  ...
  expect_true(all(comp$identical == 1))
})
