...
sheet_list_org[[6]]['dv']
sheet_list_org[[6]] <- sheet_list_org[[6]] |> mutate(dv = as.numeric(dv))
sheet_list_org[[3]]['dv']
sheet_list_org[[3]] <- sheet_list_org[[3]] |> mutate(dv = as.numeric(dv))
...
START_INFSD$dv <- START_INFSD$dv |> as.numeric
START_INFMD$dv <- START_INFMD$dv |> as.numeric
...
sum_excel <- sum_excel |> select(-2) |> # remove empty column
...
sum_excel <- sum_excel |> mutate_cond(parameter=='cl.f.obs'&route=='IV'&ss=='n',parameter='cl.obs') |>
